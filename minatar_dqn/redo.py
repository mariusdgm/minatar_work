...
def _test_register():"""Implementation credits: Florin"""
...
class ReDo:
    def __init__(self, module, inbound, outbound, tau=0.005, beta=0.1, selection_option=None, verbose=False) -> None:
        self.module = module
        self.inbound = inbound
        self.outbound = outbound
        self.tau = tau
    ...
    def redo(self):
        ...
        if self.selection_option in ["intersect", "union"]:
            smallest_weights_idx = torch.argsort(self.inbound.weight.data.abs().mean(dim=1))
            smallest_weights_idx = smallest_weights_idx[:len(idxs)]
            
            if self.selection_option == "intersect":
                idxs = torch.tensor(list(set(idxs.tolist()) & set(smallest_weights_idx.tolist())), device=idxs.device)
...
def apply_redo_parametrization(net, tau=0.025, beta=0.1, selection_option=None, verbose=False):
    """Assumes the modules are properly ordered."""
    supported_layers = (nn.ReLU, nn.LayerNorm, nn.Linear, nn.Conv2d)
    layers = [(k, v) for k, v in net.named_modules() if isinstance(v, supported_layers)]
    hndlrs = []
    ...
    for i, (_, module) in enumerate(layers):
        if isinstance(module, nn.ReLU):
            inbound, outbound = layers[i - 1], layers[i + 1]
            hook = ReDo.hook(module, inbound[1], outbound[1], tau=tau, beta=beta, selection_option=selection_option)
            hndlrs.append(hook)
            ratios.append((module, hook))
            scores.append((module, hook))
            if verbose:
