...
def collect_training_output_files(experiments_folder, append_none=False):
    ...
    config_file_path_list = search_files_containing_string(experiments_folder, "config")
    ...
    for experiment_path in training_experiment_folders:
        ...
        model_file_path_list = search_files_containing_string(
            models_folder_path, "mck", substring_location="containing"
        )
        ...
        stats_file_path_list = search_files_containing_string(
            experiment_path, "train_stats"
        )
...
def collect_pruning_output_files(experiments_folder):
    ...
    for experiment_path in pruning_experiment_folders:
        ...
        pruning_stats_file_path_list = search_files_containing_string(
            experiment_path, "pruning_results", substring_location="starting"
        )
        baseline_stats_file_path_list = search_files_containing_string(
            experiment_path, "baseline", substring_location="starting"
        )
        ...
        exp_paths[
            "pruning_stats_paths"
        ] = pruning_stats_file_path_list  # more results expected
